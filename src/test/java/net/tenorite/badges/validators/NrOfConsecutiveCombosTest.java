...
public class NrOfConsecutiveCombosTest extends AbstractValidatorTestCase {
    ...
    public void testUpgradeBadge() {
        ...
        assertThat(published).extracting("badge.gameModeId").containsExactly(GAME_MODE_ID);
        assertThat(published).extracting("badge.badgeType").containsExactly(BADGE_TYPE);
        assertThat(published).extracting("badge.level").containsExactly(4L);

        assertThat(badgeRepository.getBadgeLevel("john", BADGE)).hasValue(BadgeLevel.of("john", BADGE, 2000, 4, "newId"));
