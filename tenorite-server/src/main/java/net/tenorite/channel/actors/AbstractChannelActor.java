 ...
 */
public abstract class AbstractChannelActor extends AbstractActor {
    ...
    protected final Map<ActorRef, Slot> slots = new HashMap<>();
    ...
    public AbstractChannelActor(Tempo tempo, GameMode gameMode, boolean ephemeral) {
        this.tempo = tempo;
    ...
    protected void doOnReceive(Object message) {
    ...
    private void handleReserveSlot(ReserveSlot o) {
        ...
        if (!pending.containsKey(sender) && !slots.containsKey(sender)) {
            if (slots.size() + pending.size() < 6) {
                Optional<SlotReservationFailed> validationError = validateReservation(o);
                if (validationError.isPresent()) {
                    sender.tell(validationError.get(), self());
                }
                else {
    ...
    private void handleLeaveChannel(ActorRef actor, boolean disconnected) {
        ...
        ofNullable(slots.get(actor)).ifPresent(slot -> {
            ...
            if (!disconnected) {
                actor.tell(ChannelLeft.instance(), self());
    ...
    private void handleStartGame(StartGameMessage start) {
        findSlot(sender(), start.getSender()).ifPresent(moderator -> {
            ...
            else if (canStartGame(moderator)) {
                ActorRef self = self();
    ...
    protected void forEachSpecator(Consumer<Spectator> playerConsumer) {
        spectators.values().forEach(playerConsumer);
    }

    protected final void forEachSlot(Consumer<Slot> playerConsumer) {
        slots.values().forEach(playerConsumer);
    }

    protected void forEachSlot(Predicate<Slot> predicate, Consumer<Slot> playerConsumer) {
        slots.values().stream().filter(predicate).forEach(playerConsumer);
    }

    protected Optional<Slot> findSlot(ActorRef sender, int slot) {
        return ofNullable(slots.get(sender)).filter(p -> p.nr == slot);
    }

    protected Optional<Slot> findSlot(String name) {
    ...
    private static class AkkaScheduler implements Scheduler {
        ...
        AkkaScheduler(ActorSystem sytem) {
            this.sytem = sytem;
        }
